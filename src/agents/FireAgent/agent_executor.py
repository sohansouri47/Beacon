...
from src.agents.FireAgent.agent import FireAgent
...
class FireAgentExecutor(AgentExecutor):
    """
    Implements the AgentExecutor interface to integrate the
    fire agent with the A2A framework.
    """

    def __init__(self):
        self.agent = FireAgent()
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        try:
            ...
            error_message = f"Fire emergency error: {str(e)}"
            await updater.update_status(
                TaskState.failed,
